...
//Classe associada à cabeça do Boss, responsável por causar dano ao jogador quando o mesmo se aproxima o suficiente do Boss
public class BossHead : MonoBehaviour
{
    private readonly float damage = 26f;
    private Boss me;
    ...
    //Detetar se a cabeça do Boss entra em contacto com o jogador, quando utiliza o ataque "Headbutt"
    //Se entrar em contacto, retirar vida ao jogador equivalente ao dano deste ataque.
    private void OnTriggerEnter(Collider other)
    {
        if (other.gameObject.CompareTag("Player") && myEnemyInfo.GetHealth() > 0.0f)
        {
            ...
            Debug.LogWarning("The Boss Has Hit a Player with a Headbutt Attack");
        }
        else if (other.gameObject.CompareTag("SinglePlayer") && mySingleplayerInfo.GetHealth() > 0.0f)
        {
            ...
            playerHit.myAnimator.SetFloat("health", playerHit.GetHealth());
            playerHit.healthBar.SetHealth(playerHit.GetHealth() - damage);
            Debug.LogWarning("Boss Hit a Player with a Headbutt Attack");
        }
    }
}
